...
RUN /bin/bash -l -c "rvm alias create default 2.1.2"
...
RUN /bin/bash -l -c "nvm install v0.10.32"
RUN /bin/bash -l -c "nvm alias default v0.10.32"
...
RUN sudo chown -R nuser:nuser /aws-ecs-clockwork/

# Create Gemset
RUN /bin/bash -l -c "rvm gemset create aws-ecs-clockwork"
RUN /bin/bash -l -c "rvm use 2.1.2@aws-ecs-clockwork --default"
